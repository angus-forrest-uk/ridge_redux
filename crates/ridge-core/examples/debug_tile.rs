...
fn main() {
    ...
    match src.load_or_download(44, -72) {
        Ok(t) => {
            ...
            println!("Mt Washington (44.2705, -71.3030) = {} m", t.elevation(44.2705, -71.3030));
        }
        Err(e) => println!("download failed: {e}"),
    }
    let idx = src.index();
    match idx {
        Ok(m) => println!("index size: {} (N44W072 -> {})", m.len(), m.get("N44W072.hgt").map(|s| &s[..s.len().min(60)]).unwrap_or("MISSING")),
