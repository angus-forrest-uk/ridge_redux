...
fn main() {
    ...
    for deg in [0, 15, 30, 45, 60, 75, 90] {
        ...
        for i in 0..n {
            ...
            for c in 0..p {
                ...
                if v.is_finite() {
                    ...
                    let y = v + baseline;
                    if y > ymax { ymax = y; }
                    if (c as f64) < xmin { xmin = c as f64; }
                    if c as f64 > xmax { xmax = c as f64; }
                }
            }
            if has_data && baseline < ymin { ymin = baseline; }
        }
        ...
        eprintln!("  content x-extent: {:.0} of {} cols  (apparent width {:.0}%)",
            xmax - xmin, p as f64, 100.0 * (xmax - xmin) / (p as f64 - 1.0));
    }
}
