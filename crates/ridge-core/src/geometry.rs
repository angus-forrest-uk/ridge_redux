...
impl RidgeScene {
    ...
    pub fn from_grid(processed: &Array2<f64>, bbox_ratio: f64, size_scale: f64) -> RidgeScene {
        ...
        // matplotlib autoscale with 5% margins, but over the WHOLE window
        // rather than over the cells that happen to hold land. Upstream lets
        // the drawn data drive autoscale, and matplotlib drops non-finite
        // points, so any all-water column or row leaves the frame: a bbox
        // covering a tile with no data renders its coastline alone, and the
        // `water_ntile` knob rescales the picture as well as the ridges.
        // Pinning the frame keeps water, lakes and voids in their place - the
        // missing tile shows as flat water and the composition follows the
        // requested bbox. `fixtures/legacy/frame.json` (regenerated by
        // `just fixtures-legacy`) records both frames for the same input, so
        // the deviation is measured rather than assumed.
        let xmax_data = (ncols - 1) as f64;
        let ymax_data = rows
            .iter()
        ...
        // The window floor is the last row's baseline. Upstream stops at the
        // last baseline that still carries data, which is what crops the
        // water rows away from the bottom of the picture.
        let ymin = -LINE_SPACING * (nrows - 1) as f64;
...
mod tests {
    ...
    #[test]
    fn frame_is_the_whole_window_not_the_land() {
        // Rows 0-1 and columns 0-2 are pure water (NaN). The frame must cover
        // them anyway: the picture follows the requested window, so masking
        // cannot rescale it.
        ...
        let scene = RidgeScene::from_grid(&grid, 1.0, DEFAULT_SIZE_SCALE);
        // x: columns 0..9, 5% of the span a side.
        assert_eq!(
            scene.layout.xlim,
            [-9.0 * AXES_MARGIN, 9.0 + 9.0 * AXES_MARGIN]
        );
        // y: window floor at the last baseline (-30), up to the highest drawn
        // point (row 2, baseline -12, value 20), 5% a side.
        assert_eq!(
            scene.layout.ylim,
    ...
    fn water_masking_never_moves_the_frame() {
        ...
        let a = RidgeScene::from_grid(&land, 1.0, DEFAULT_SIZE_SCALE);
        let b = RidgeScene::from_grid(&half_water, 1.0, DEFAULT_SIZE_SCALE);
        assert_eq!(
            a.layout, b.layout,
