...
impl RidgeScene {
    ...
    pub fn from_grid(
        ...
        // Where the axes start and stop. `Frame::Window` frames the whole
        // requested window, so water and voids keep their share of the picture
        // and no masking-related setting can move it. `Frame::Land` reproduces
        // the legacy plot instead: matplotlib autoscales over the points it
        // draws and drops the non-finite ones, so all-water columns and rows
        // fall out of the frame and `water_ntile` rescales the picture.
        // `fixtures/legacy/frame.json` records the legacy run for the same
        // input, so the difference is measured rather than assumed.
        let (xmin, xmax_data, ymin) = match frame {
            Frame::Window => (0.0, (ncols - 1) as f64, -LINE_SPACING * (nrows - 1) as f64),
            Frame::Land => {
                let (first, last) = rows
                    .iter()
                    .flat_map(|row| row.y.iter().enumerate())
                    .filter(|&(_, y)| y.is_finite())
                    .fold((usize::MAX, 0usize), |(first, last), (c, _)| {
                        (first.min(c), last.max(c))
                    });
                let lowest = rows
                    .iter()
                ...
                if first == usize::MAX {
                    // Nothing drawable even by matplotlib's reckoning: fall
                    // back to the theoretical frame.
                    (0.0, (ncols - 1) as f64, -LINE_SPACING * (nrows - 1) as f64)
                } else {
                    (first as f64, last as f64, lowest)
                }
            }
        };
        let ymax_data = rows
            .iter()
            .flat_map(|row| row.y.iter())
            .copied()
            .filter(|y| y.is_finite())
            .fold(f64::NEG_INFINITY, f64::max);
        // No drawable content at all: fall back to the theoretical frame.
        let ymax_data = if ymax_data.is_finite() {
            ymax_data
        } else {
            vmax
        };
        let dx = (xmax_data - xmin) * AXES_MARGIN;
...
/// How the figure frame is fitted around the processed grid.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum Frame {
    /// The whole requested window: water, lakes and voids keep their place, so
    /// no mask can move the frame. This is the default.
    #[default]
    Window,
    /// matplotlib's tight autoscale around the cells it actually draws, which
    /// is what the legacy plot does: any all-water column or row drops out of
    /// the frame, so `water_ntile` rescales the picture as well as the ridges.
    Land,
}
...
mod tests {
    ...
    /// A grid whose top two rows, bottom row and left three columns are all
    /// water (NaN): the shapes that exposed the frame clipping.
    fn water_edged_grid() -> Array2<f64> {
        Array2::from_shape_fn((6, 10), |(r, c)| {
            if r < 2 || r == 5 || c < 3 {
                f64::NAN
            } else {
                20.0
    ...
    #[test]
    fn window_frame_is_the_whole_requested_window() {
        // The picture follows the requested window, so masking cannot rescale
        // it: every column keeps its place...
        let scene =
            RidgeScene::from_grid(&water_edged_grid(), 1.0, DEFAULT_SIZE_SCALE, Frame::Window);
        assert_eq!(
            scene.layout.xlim,
            [-9.0 * AXES_MARGIN, 9.0 + 9.0 * AXES_MARGIN]
        );
        // ...and so does every row, down to the last baseline (-30), even the
        // all-water one. The top is still the highest drawn point (row 2,
        // baseline -12, value 20).
        assert_eq!(
            scene.layout.ylim,
            [-30.0 - 38.0 * AXES_MARGIN, 8.0 + 38.0 * AXES_MARGIN]
        );
    }

    #[test]
    fn land_frame_reproduces_the_legacy_crop() {
        // The legacy plot lets matplotlib autoscale around the points it
        // draws, so the water columns and the all-water bottom row leave the
        // frame entirely.
        let scene =
            RidgeScene::from_grid(&water_edged_grid(), 1.0, DEFAULT_SIZE_SCALE, Frame::Land);
        // x: only columns 3..9 carry land.
        assert_eq!(
            scene.layout.xlim,
            [3.0 - 6.0 * AXES_MARGIN, 9.0 + 6.0 * AXES_MARGIN]
        );
        // y: up to the last baseline that still carries land (row 4, -24).
        assert_eq!(
            scene.layout.ylim,
            [-24.0 - 32.0 * AXES_MARGIN, 8.0 + 32.0 * AXES_MARGIN]
        );
    }

    #[test]
    fn water_masking_never_moves_the_frame() {
        // The regression behind "water_ntile clips the frame back": masking a
        // band to water used to shrink the frame onto the remaining land, so
        // raising water_ntile rescaled the picture, and a bbox covering a tile
        // with no data collapsed onto its coastline.
        let land = Array2::from_elem((5, 7), 40.0);
        let half_water =
            Array2::from_shape_fn((5, 7), |(_, c)| if c < 3 { f64::NAN } else { 40.0 });
        let a = RidgeScene::from_grid(&land, 1.0, DEFAULT_SIZE_SCALE, Frame::Window);
        let b = RidgeScene::from_grid(&half_water, 1.0, DEFAULT_SIZE_SCALE, Frame::Window);
        assert_eq!(
            a.layout, b.layout,
            "the frame is the window, whether the water is there or not"
        );
    }
}
